...
@Component({
  ...
  template: `
    <ul cdkDropList (cdkDropListDropped)="dragItem($event)" class="current-list">
      @for (entry of internalList(); track entry['listItemId']) {
        <li class="list-item" cdkDrag [cdkDragData]="entry.listItemId">
...
export class ControlListComponent<T extends FormControl & { listItemId: string }> {
  IconEnum = IconEnum;
  list = input.required<FormRecord>();

  internalList = linkedSignal(() => {
    const controls = (this.list() as FormRecord<T>).controls;
    const arrList: T[] = [];
    let idx = 0;
    for (const key in controls) {
      controls[key]['listItemId'] = `item${idx}`;
      arrList.push(controls[key] as T);
      idx++;
    }
  ...
  protected dragItem(event: CdkDragDrop<string>) {
    this.internalList.update(order => {
